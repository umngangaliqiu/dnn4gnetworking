...
def cvx_dc(p, q, r_vector, r_matrix, x_matrix, a_matrix, a_inv, a0, v0, bus, nm):
    v_max = bus[1:, 3]
    v_min = bus[1:, 4]
...
def cvx_ac(p, q, r_vector, r_matrix, x_matrix, a_matrix, a_inv, a0, v0, bus, nm):
    v_max = 1.5*bus[1:, 3]
    v_min = .6*bus[1:, 4]
    ...
    obj_ac = cp.Minimize(np.hstack(np.zeros((nm, 1)), r_vector.T)*np.hstack(q_flow.T, l_flow.T).T)
